...
@section('content')
    ...
    <div class="mb-5 grid gap-4 md:grid-cols-3">
        ...
        <div class="stat-card">
            <div>
                <p class="stat-kicker">Produk terjual</p>
                <p class="stat-value">{{ number_format($stats['products'] ?? 0) }}</p>
                <p class="stat-hint">Varian unik</p>
            </div>
        ...
        <div class="table-wrap">
            <table class="{{ ($exporting ?? false) ? 'data-table' : 'list-table' }}">
                <thead>
                    ...
                    @unless ($exporting ?? false)
                        <tr class="filter-row">
                            <th></th>
                            <th>
                                <input form="product-sales-filters" class="col-filter" type="search" name="name" value="{{ $filters['name'] ?? $filters['search'] ?? '' }}" placeholder="Nama..." onchange="this.form.submit()">
                            </th>
                            <th></th>
                            <th></th>
                            <th></th>
                ...
                <tbody>
                    @forelse ($rows as $row)
                        <tr>
                            ...
                            <td>
                                @unless ($exporting ?? false)
                                    @if ($row->product_id)
                                        <a href="{{ route('products.index', ['modal' => 'view', 'product' => $row->product_id]) }}" class="font-semibold hover:underline">{{ $row->name }}</a>
                                    @else
                            ...
                            @unless ($exporting ?? false)
                                <td class="col-actions">
                                    <div class="flex items-center justify-end gap-1.5">
                                        @if ($row->product_id)
                                            <a href="{{ route('products.index', ['modal' => 'view', 'product' => $row->product_id]) }}" class="table-action" title="Lihat">
                    ...
                    @empty
                        <tr>
                            <td colspan="{{ ($exporting ?? false) ? 4 : 5 }}" class="py-16 text-center text-sm text-slate-400">Tidak ada penjualan produk yang cocok dengan filter ini.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
