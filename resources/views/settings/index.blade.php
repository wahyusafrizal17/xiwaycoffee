...
@section('content')
    <div class="mb-5 grid gap-4 md:grid-cols-3">
        <div class="stat-card">
            <div>
                <p class="stat-kicker">Biaya layanan</p>
                <p class="stat-value">{{ number_format($stats['tax_rate'], 2) }}%</p>
                <p class="stat-hint">Dipakai hitung invoice (label tanpa %)</p>
            </div>
    ...
    <form method="POST" action="{{ route('settings.update') }}" class="card overflow-hidden">
        ...
        <div class="space-y-8 px-5 py-6">
            <section>
                <h6 class="mb-4 text-sm font-semibold text-heading">Biaya layanan</h6>
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        <label class="label">Tarif biaya layanan (%)</label>
                        ...
                        <p class="mt-1 text-xs text-muted">Ditampilkan di invoice sebagai “Biaya Layanan” (tanpa persen).</p>
                    </div>
                    <div>
